...
/// </summary>
public class STObjectPoolTests : IDisposable
{
    private readonly List<STObjectPool<TestObject>> _poolsToDispose = new();
    ...
    private STObjectPool<TestObject> CreatePool(
        ...
        int maxSize = 100)
    {
        var pool = new STObjectPool<TestObject>(
    ...
    {
        // Act
        // Create a pool with 5 pre-allocated objects
        var pool = CreatePool(defaultCapacity: 5);

        // Assert
        // Pool should be created with all objects ready and inactive
        Assert.NotNull(pool);
    ...
    public void Constructor_WithNullCreateFunc_ThrowsArgumentNullException()
    {
        ...
        Assert.Throws<ArgumentNullException>(() => new STObjectPool<TestObject>(null!));
    }
    ...
    {
        // Arrange
        // With collectionCheck enabled, double-release is detected and throws
    ...
    public void PooledObject_Dispose_ReleasesBackToPool()
    {
        ...
        // Act
        // Disposing the PooledObject wrapper automatically releases it
    ...
    {
        // Arrange
        // Test that count properties accurately track pool state
        var pool = CreatePool(defaultCapacity: 5);

        // Initial state - all objects inactive
        Assert.Equal(5, pool.CountAll);
        Assert.Equal(5, pool.CountInactive);
        Assert.Equal(0, pool.CountActive);

        // Get 3 objects - moves them from inactive to active
        var obj1 = pool.Get();
        var obj2 = pool.Get();
        var obj3 = pool.Get();
        ...
        // Release 1 object - moves it from active to inactive
        pool.Release(obj1);
    ...
    {
        // Arrange
        // Use actionOnGet to initialize object state when retrieved
        var pool = CreatePool(
        ...
        // Assert
        // Object should be initialized by the callback
    ...
    [Fact]
    public void MultipleGetsAndReleases_MaintainCorrectCounts()
    {
        // Arrange
        var pool = CreatePool(defaultCapacity: 3);
        var objects = new List<TestObject>();

        // Act - Get multiple objects
        for (int i = 0; i < 5; i++)
        {
        ...
        Assert.Equal(5, pool.CountAll);
        Assert.Equal(0, pool.CountInactive);
        Assert.Equal(5, pool.CountActive);

        // Act - Release all objects
    ...
    [Fact]
    public void CreateFunc_CalledForEachNewObject()
    {
        // Arrange
        var createCount = 0;
        var pool = CreatePool(
            ...
            {
                createCount++;
                return new TestObject { Id = createCount };
            },
            defaultCapacity: 2);

        // Initial objects created in constructor
        Assert.Equal(2, createCount);

        // Act - Get more than default capacity
        var obj1 = pool.Get();
        var obj2 = pool.Get();
        var obj3 = pool.Get(); // This should create a new one

        // Assert
        Assert.Equal(3, createCount); // 2 from constructor + 1 new
